...
class StatusTest(unittest.TestCase):
    ...
    def test_glyph_in_ascii_list(self):
        ...
        self.assertIn("○", out)
        self.assertIn("●", out)
        self.assertIn("○ new · ● in progress · ✕ closed", out)   # `open`→"new", `active`→"in progress"
    ...
    def test_glyph_in_markdown(self):
        ...
        self.assertIn("_○ new · ● in progress · ✕ closed_", out)   # `open`→"new", `active`→"in progress"
    ...
    def test_glyph_in_detail(self):
        ...
        self.assertIn("○ NEW", ts._format_detail(o, "sx"))   # `open` displays as "new"
        self.assertIn("● IN PROGRESS", ts._format_detail(a, "sx"))
    ...
    # --------------------------------------------------- open→"new" relabel (1.21)
    def test_status_display_maps_open_to_new(self):
        # `open` DISPLAYS as "new"; active/closed unchanged; the stored value stays.
        self.assertEqual(ts.status_display("open"), "new")
        self.assertEqual(ts.status_display("active"), "in progress")
        self.assertEqual(ts.status_display("closed"), "closed")

    def test_status_legend_uses_new_label(self):
        self.assertIn("○ new", ts.status_legend())
        self.assertNotIn("○ open", ts.status_legend())

    def test_new_is_input_alias_for_open_status_cmd(self):
        # `new` is accepted wherever `open` is — it maps to the stored `open`.
        self.assertEqual(ts.normalize_status_input("new"), "open")
        t = self._seed("Alias me", status="active")
        buf = io.StringIO()
        with redirect_stdout(buf):
            ts.cmd_status(_Args(task=str(t["seq"]), value="new"))
        self.assertEqual(ts.task_status(ts.load_task(t["id"])), "open")   # stored open
        self.assertIn("○ new", buf.getvalue())                            # displayed new

    def test_new_is_input_alias_for_open_on_create(self):
        # new_task accepts "new" as the alias and stores the canonical "open".
        self.assertEqual(ts.new_task("x", "", status="new")["status"], "open")
    ...
    def test_status_command_reports_when_no_value(self):
        ...
        self.assertIn("status: ○ new", buf.getvalue())   # `open` displays as "new"
        self.assertEqual(ts.task_status(ts.load_task(t["id"])), "open")
