...
class BriefingTest(unittest.TestCase):
    ...
    def test_extract_prs_github_and_ado_dedup(self):
        ...
        ado = "https://dev.azure.com/companyname/Proj/_git/Repo/pullrequest/123"
        t = self._task()
        t["log"] = [
    ...
    def test_detail_shows_briefing_when_present(self):
        ...
        # State (next) + Artifacts (files · PRs · repos), digest-first.
        t = self._task()
        ...
        self.assertIn("State (next):", detail)
        ...
        self.assertLess(detail.index("State (next):"), detail.index("Recent activity"))

    def test_detail_omits_briefing_when_empty(self):
        t = self._task()
        detail = ts._format_detail(t, "sess")
        self.assertNotIn("State (next):", detail)
        self.assertNotIn("Artifacts:", detail)

    # -- displayed timestamps render in the system local timezone --------------

    def _with_tz(self, zone, fn):
        """Force the process timezone to `zone` for the duration of `fn()` (POSIX
        tzset), restoring whatever was set before — so the assertion holds
    ...
    def test_local_iso_converts_stored_utc_to_local(self):
        # America/New_York is never at +00:00, so this holds whatever the host's
        # own timezone happens to be.
        converted = self._with_tz("America/New_York",
                                   lambda: ts._local_iso("2026-07-02T15:00:00+00:00"))
        self.assertEqual(converted, "2026-07-02T11:00:00-04:00")

    def test_local_iso_passes_through_unparseable_or_empty(self):
        self.assertEqual(ts._local_iso(""), "")
        self.assertEqual(ts._local_iso("not-a-timestamp"), "not-a-timestamp")

    def test_created_line_renders_in_local_tz_not_utc(self):
        def _created():
            t = self._task()
            detail = ts._format_detail(ts.load_task(t["id"]), "sess")
            return detail

        detail = self._with_tz("America/New_York", _created)
        self.assertIn("Created:", detail)
        self.assertNotIn("+00:00", detail)
