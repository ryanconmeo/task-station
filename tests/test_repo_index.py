    ...
    def test_ado_remote(self):
        self.assertEqual(
            repo_index.parse_ado_project(
                "https://companyname@dev.azure.com/companyname/Projectname/_git/projectname-api"),
            "Projectname")
...
class Overrides(unittest.TestCase):
    ...
    def test_merge_overrides_win(self):
        repos = [{"name": "projectname-api", "status": "active", "stack": ["dotnet"]}]
        ov = {"projectname-api": {"summary": "Projectname billing API",
                           "keywords": ["billing", "invoices"],
                           "domain": ["finance"],
                           "status": "stale"}}
        merged = repo_index.merge_overrides(repos, ov)
        r = merged[0]
        self.assertEqual(r["summary"], "Projectname billing API")
...
class Match(unittest.TestCase):
    def test_obvious_term_ranks_first(self):
        repos = [
            ...
            {"name": "projectname-billing", "keywords": ["invoices", "billing"], "domain": ["finance"], "stack": ["dotnet"], "path": "/w/projectname-billing"},
            {"name": "marketing-site", "keywords": [], "domain": [], "stack": ["node"], "path": "/w/marketing-site"},
        ]
        ranked = repo_index.match("billing invoice work", repos)
        self.assertEqual(ranked[0]["name"], "projectname-billing")
