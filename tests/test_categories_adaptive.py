...
class NewDefaults(_Base):
    ...
    def test_palette_follows_swapped_slots(self):
        ...
        self.assertIn(white_bg, c.tint_escape("white", "iterm"))
        self.assertIn(silver_bg, c.tint_escape("silver", "iterm"))
        self.assertIn(white_bg, c.tint_escape("DESIGN", "iterm"))
        self.assertIn(silver_bg, c.tint_escape("TOOLING", "iterm"))
...
class SlotDeterminesEmoji(_Base):
    ...
    def test_override_without_dot_inherits_slot_emoji(self):
        self._write_config({"categories": {"green": {"tag": "PROJECT", "label": "project work"}}})
        c = self._reload()
        self.assertEqual(c.CATEGORIES["green"]["tag"], "PROJECT")
    ...
    def test_explicit_dot_still_honored(self):
        self._write_config({"categories": {"green": {"dot": "⚡", "tag": "PROJECT", "label": "project"}}})
        c = self._reload()
        self.assertEqual(c.CATEGORIES["green"]["dot"], "⚡")

    def test_user_project_and_migration_overrides_win(self):
        ...
        self.assertEqual(c.resolve("PROJECT"), "green")
...
class CategoryGuide(_Base):
    """1.28.0 I: per-slot "when to use" guidance for the HTML board's expandable rows."""

    def test_known_key_returns_its_sentence(self):
        c = self._reload()
        self.assertEqual(c.category_guide("green"), "Feature / product coding.")
        self.assertEqual(c.category_guide("red"), "Fixing a defect / broken behaviour.")
        # resolves via the same aliases as resolve() — a [TAG] / label / emoji works.
        self.assertEqual(c.category_guide("FEATURE"), "Feature / product coding.")

    def test_every_slot_has_a_guide(self):
        c = self._reload()
        for key in c.all_keys():
            self.assertTrue(c.category_guide(key), "missing guide for %s" % key)

    def test_unknown_key_returns_empty(self):
        c = self._reload()
        self.assertEqual(c.category_guide("not-a-colour"), "")
        self.assertEqual(c.category_guide(""), "")
        self.assertEqual(c.category_guide(None), "")


class DefaultTagLabel(_Base):
    """1.29.0 (part 3): default_tag_label exposes the SHIPPED default for a slot so the
    board can show what it was before a user override."""

    def test_known_slot_returns_shipped_default(self):
        c = self._reload()
        d = c.default_tag_label("green")
        self.assertEqual(d, {"dot": "🟢", "tag": "FEATURE", "label": "feature work"})
        # resolves via the same aliases as resolve() — a [TAG] works too.
        self.assertEqual(c.default_tag_label("FEATURE")["tag"], "FEATURE")

    def test_returns_shipped_default_even_when_overridden(self):
        # the snapshot is captured BEFORE _apply_overrides, so an override of the slot
        # does NOT change what default_tag_label reports.
        self._write_config({"categories": {"green": {"tag": "SHIP", "label": "shipping"}}})
        c = self._reload()
        self.assertEqual(c.CATEGORIES["green"]["tag"], "SHIP")     # override applied
        self.assertEqual(c.default_tag_label("green"),
                         {"dot": "🟢", "tag": "FEATURE", "label": "feature work"})

    def test_unknown_slot_returns_none(self):
        c = self._reload()
        self.assertIsNone(c.default_tag_label("not-a-colour"))
        self.assertIsNone(c.default_tag_label(""))
        self.assertIsNone(c.default_tag_label(None))
