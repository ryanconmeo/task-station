...
class RenderMarkdownTest(unittest.TestCase):
    ...
    def test_two_tables_columns_and_seq(self):
        ...
        self.assertIn("|:-:|--:|------|----------|--------|----------|", out)
        # STATUS is its own leading column: ● active · ○ new · ✕ closed (`open`
        # displays as "new"). The `#` cell now holds the bare seq number only.
    ...
    def test_legend_line(self):
        ...
        out = ts._format_list_md()
        self.assertIn("_○ new · ● in progress · ✕ closed_", out)
    ...
    def test_commands_footer_as_fenced_help_block(self):
        ...
        self.assertIn("/todo config [flags]    open settings", out)   # 1.54.0: deduped to the /todo form
        ...
        self.assertNotIn("\n- /todo", out)
        # 1.59.0: bare ON → the two-line on-state note trails the fenced block, the
        # /task-station: prefix statement on its own line.
        self.assertIn(
            "bare-cmds is on — /todo, /done, /save, /heal, /pin, /history, /repos work directly.\n"
            "The /task-station: prefix also always works.", out)

    def test_commands_footer_bare_off_uses_namespaced_forms(self):
        self._set_bare(False)
        self._seed("Some task")
        out = ts._format_list_md()
        self.assertIn("```\n/task-station:todo", out)
        self.assertIn("/task-station:todo config [flags]", out)
        self.assertIn("bare-cmds is off — use the /task-station: prefix (shown).", out)
        self.assertIn("/task-station:config --bare-cmds on", out)
