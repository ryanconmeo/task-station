...
class DataDirResolution(unittest.TestCase):
    ...
    def test_explicit_override_wins(self):
        os.environ["TASK_STATION_HOME"] = "/tmp/td-home"
        os.environ["CLAUDE_CONFIG_DIR"] = "/tmp/cfg"
        self.assertEqual(paths.data_dir(), "/tmp/td-home")

    def test_config_dir_then_task_station_data(self):
        os.environ["CLAUDE_CONFIG_DIR"] = "/tmp/cfg"
        self.assertEqual(paths.data_dir(), "/tmp/cfg/task-station-data")
    ...
    def test_default(self):
        self.assertEqual(paths.data_dir(),
                         os.path.expanduser("~/.claude/task-station-data"))
