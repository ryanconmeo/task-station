---
description: claude-todo doctor + installers (delegation policy, terminal tint profiles).
...
The block above is the live output of `claude-todo setup` — either the status report (no arguments) or the result of an installer flag. Present it to the user verbatim; do not editorialize.
...
- `--workspace-dirs <a:b>` → same as `/claude-todo:config --workspace-dirs` (offered here too for the guided flow).

Unlike `/claude-todo:config` (which edits values the plugin owns), `setup` performs and reverses changes to your wider environment.
