...
argument-hint: "[--workspace-dirs a:b | --categories [edit] | --auto-categories on|off | --enable/--disable <key> | --tint-theme auto|dark|light | --title on|off | --bare-cmds on|off | --update-check on|off | --desktop-bridge on|off | --policy on|off]"
allowed-tools: Bash
...
- `--auto-categories on|off` → when on (default), assigning a task to a category that isn't on the board **auto-enables** that slot so the board grows itself; off freezes the set (also via `TASK_STATION_AUTO_CATEGORIES=off`). The categoriser always considers the full taxonomy regardless.
