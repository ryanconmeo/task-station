---
description: View or change claude-todo settings (workspace dirs, categories).
...
The block above is the live output of `claude-todo config` — either the settings board (no arguments) or the result of a `--flag` change. Present it to the user verbatim in a code block; do not editorialize.
...
These are stored values the plugin owns. For actions that touch your wider environment (CLAUDE.md policy, Terminal tint profiles), use `/claude-todo:setup` instead.
