...
<p>
  <img alt="version" src="https://img.shields.io/badge/version-3.11.0-blue">
...
`invoke` spawns a session **already attached to the child's own task**, so its SessionStart injects that task's digest and the ask carries the request only — there is no brief to get wrong. `--role` (scout · implementer · reviewer · grader) picks the child's model from a small role table, and its permission mode **only when that mode narrows** what the child may do: a role may restrict and may never replace, so anything else stays silent and inherits your configured default. A role's bare model alias also reclaims the parent's `[1m]` window when both name the same family — otherwise an orchestrator at 1M hands its child one fifth of the context without anyone asking.
