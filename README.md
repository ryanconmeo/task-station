...
<p>
  <img alt="version" src="https://img.shields.io/badge/version-3.60.0-blue">
...
- **A proactive nudge to checkpoint *before* compaction.** As a session fills, Task Station prompts a full structured `/todo save` from full context — a better, task-shaped compaction than the generic auto-summary — so a fresh `/todo <n>` resumes from your digest, not the harness summary. The default trigger is a **percentage of your real context window**, measured from the transcript's actual `usage` block: it fires once the measured context reaches `--checkpoint-pct` % (default `65`) of your **detected** context window. The window is resolved from the session itself — the harness's own reported size first, then the Claude Code model selection's `[1m]` marker, then the transcript's model — so a 1M session is measured against 1,000,000 and a 200k session against 200,000, with nothing to keep in sync by hand. `--context-window <tokens>` is an **override** for the cases detection cannot reach; `--context-window auto` drops it again. The nudge names the measured pressure AND the source it was measured against — e.g. *context ~68% (~136k/200k tokens, window source: harness)* — and when a stored override disagrees with what the session detects, it says so. `task-station window` prints the resolution on its own. Set `--checkpoint-pct 0`/`off` to disable it. A legacy absolute fallback, `--checkpoint-at <tokens>` (default `off`), fires off a transcript-size token *estimate* when a real measurement isn't available — prefer the pct trigger. It fires once per pressure episode — it won't spam you if you defer. Auto-compaction can't be reliably prevented; the win isn't blocking compaction, it's keeping the durable digest current so the *resume* is structured.
...
- **Save before the window gets heavy (~60–70%), never after auto-compaction.** A digest written post-compact is a summary of a summary — the detail is already gone. The proactive percentage nudge (`--checkpoint-pct`, default `65`% of the **detected** window) fires precisely while full context is still intact; act on it rather than deferring into a compaction.
...
### Maintenance timing — when, not just whether (opt-in)

Three maintenance actions — the checkpoint, the reconcile pass, and the handoff — used to fire on a **count**. A count says an action is *owed*. It says nothing about whether **now** is a good time, and the two moments the product picked were the two worst: session start (a heal before any work exists to reconcile, charged on top of the digest read) and mid-task (a nag in the middle of a thought, dismissed reflexively — which is how a nag becomes furniture).

The right moment is a **work boundary**: a turn ending with nothing in flight. At a boundary the summary a checkpoint needs is still in context and nearly free; an hour later it costs a re-read.

- **The boundary is composed, not invented.** The `Stop` hook already knows a turn is ending; the [pickup rail](#learning-a-child-finished--the-pickup-rail) already knows whether a child's work is outstanding. "Turn ending **and** no unclaimed pickup **and** no undelivered order **and** no half-done merge in the working tree" is the signal.
- **Two classes, and the line is enforced in code.** `config --boundary-maintenance on` lets the **AUTO** class act at that boundary and *report what it did* — the mechanical heal ops that are deterministic and reversible with a named undo, taken under the same pre-heal backup `heal --apply` takes. The **PROMPT** class always asks. **Nothing outward, nothing that ends a session, and no merge is ever automatic**: a merge summary has to be true of all its members at once, and when it is not there is no verb that unsays it.
- **The handoff finally has a trigger, and it is a floor.** Past a certain budget there is not enough room left to *write* a handoff, so it fires while enough remains to do it well — and it **refuses to prompt while the checkpoint is stale**, because handing over a stale record is worse than handing over late.
- **`task-station timing` prints the whole verdict and writes nothing**, so the scheduler can be read before it is allowed to act. **`task-station window`** prints which context window this session is measured against and which source said so — the surface that exists because a bare denominator was wrong by 5x for months and nothing could have said so.

**Off by default.** It is the one switch that lets a machine write to your record with nobody watching.
