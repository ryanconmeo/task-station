...
<p>
  <img alt="version" src="https://img.shields.io/badge/version-1.19.0-blue">
...
- **A visual board.** `/todo board` (or `task-station board`) writes a single‑file HTML `/todo board` to `<data_dir>/board.html` — the **same Open / Closed grid as the terminal `/todo` board** (status · # · task · category · effort · activity), every row carries a **mini progress bar + `N/M`** when it has steps, its **left stripe is the category's background colour**, and on hover an overflowing **title auto‑scrolls** to reveal itself. Each row is **expandable** (native `<details>`) to reveal — leading with the **full, untruncated title** — the digest (goal · next/standing · steps checklist with rollup · decisions · files · **PRs, each on its own line with an optional description** · repos), then two clearly‑labeled commands: **Open the task** (`/todo <n>` — attaches it to *this* session, the recap) above **Resume the session** (`cd … && claude --resume …` — jumps back into the original working session; pinned is folded right into this label), workers in a separate de‑emphasised subsection, and **last** the full summary as **light Markdown** (HTML‑escaped first) in a **scroll‑capped** box. An expanded row gets a **distinct background** so its bounds are obvious. A **light/dark toggle** (top‑right) flips both palettes in place and **persists to `localStorage`** so the auto‑refresh never resets it; category colours and page chrome track your **active theme + resolved variant**. A **Help** panel lists the `/todo` commands and current config. The board uses **inline `<script>`/`<style>` only — no server, no dependencies, and no external assets** (escaped content stays inert even with JS present). Add `--open` to pop it in your browser (macOS). Want an open tab to stay live? `config --board-autorefresh on` adds a meta‑refresh and quietly regenerates the board on each Stop — still no network.
...
| **prs** | Stored PRs as `{url, desc}` (an optional per‑PR description), **merged** with the links auto‑extracted from the log/summary/state (deduped by url). | `update --pr '<url>' [--pr-desc '<text>']` (upsert by url; `--pr-desc` alone updates the most‑recent PR) |
...
| `--board-autorefresh [on\|off]` | on/off | off | Opt‑in: an open `/todo board` tab stays live — a `<meta http-equiv="refresh">` tag plus a Stop‑hook regen of an already‑opened `board.html`. No JavaScript, no network. `TASK_STATION_BOARD_AUTOREFRESH` overrides. |
