...
# `stack_map`, a GENERATED module (regenerate via `tools/gen_stack_map.py`) with a
# curated top-~40 common stacks and the tool's ergonomic labels (python/node/
# dotnet/sql/...). Unknown extensions aren't listed — detection just skips them.
...
def _ext_histogram_stacks(files):
    ...
    extension (curated `EXT_TO_STACK`), falling back to its basename for
