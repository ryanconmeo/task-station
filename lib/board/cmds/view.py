...
def cmd_guidance(a):
    ...
    lines.extend([
        ...
        "should own the work)",
        "  sync  [--init [DIR]] [--init-share [DIR]] [--dir DIR] [--status] "
        "[--dry-run] [--no-net]   — "
        "TWO DESTINATIONS, AND THEY ARE NOT THE SAME THING. --init makes a BACKUP "
        "exchange: EVERY task, unfiltered, no reader but you — filtering it would kill "
        "the durability guarantee, so it is never filtered. --init-share makes a SHARE "
        "exchange: a CHOSEN SUBSET, readable by others, and PRIVATE BY DEFAULT ENFORCED "
        "ON WRITE — a task reaches it only because a sharing rule on its brain names an "
        "audience (`brains share <brain> --with <alias>` is the only thing that widens "
        "it), and with no rule there is NO FILE, not a hidden one. Filtering on WRITE "
        "rather than on read is the safety property: a read-side filter leaves the bytes "
        "in a repository other people can read. What a shared task publishes is an "
        "ALLOW-LIST (identity + goal + step counts at the default trail visibility) — "
        "never your cost, sessions, file paths or narrative, which are never BUILT into "
        "a share view rather than stripped from one. `sync` runs both, BACKUP FIRST. An "
        "exchange declares its own kind in exchange.json; aiming a backup run at a share "
        "exchange is REFUSED, and pointing both at one directory is refused outright. "
        "Un-sharing RETRACTS the published file. "
        "EXCHANGE TASKS with this owner's other machines. Each STATION writes ONE "
        "directory in the exchange (owners/<owner>/station-<n>/) and reads every other, "
        "so two machines never write the same path and a merge conflict is IMPOSSIBLE "
        "rather than merely unlikely. Inside a task the merge is PER FIELD: lists union, "
        "a matched element's flags merge (superseded on one machine + pinned on the "
        "other applies BOTH), and a scalar takes the newest BY THAT FIELD'S OWN "
        "timestamp while preserving what it replaced in <field>_history. `seq` NEVER "
        "crosses — it is machine-local, and a relation edge is stripped of it on export "
        "and re-stamped locally on import. THE REPORT IS A THREE-ROW VERDICT "
        "(Mechanical / Judgment / Heal-due) because SYNC DOES MECHANICS AND HEAL DOES "
        "MEANING: a union can leave two contradictory decisions both current, so "
        "'0 conflicts' is not a statement that the record makes sense. --init creates "
        "the exchange + this station's partition and a LOCAL repo; it creates NO REMOTE "
        "and contacts no network, and with no remote `sync` commits locally and sends "
        "nothing. --status names every partition present; --dry-run writes nothing. "
        "See docs/SYNC.md.",
        ...
        "task and TOLD TO READ IT: the launch prompt names `search --detail <n>`, so the "
        "ask carries the REQUEST ONLY. Attachment is a pointer, not a delivery — nothing "
        "is loaded into the child at session start, and every line that used to say "
        "otherwise was talking it out of the one read it needed (#583). "
        "There is no brief to get wrong; an ask long enough to be context is "
        ...
        "  relay  [--task <ref>] [--spawn] [--force] [--model M] [--cwd D] "
        "[--print-command] [--json]   — SESSION SUCCESSION. Bare, it REPORTS and writes "
        "NOTHING: the measured occupancy (from the transcript's own usage block), the two "
        "numbers the policy runs on, the verdict, and what a handoff would still lose. The "
        "verdict is keep-going below the trigger (succession_pct, 65%) · relay above it "
        "with the handoff reserve intact (succession_reserve, 40k tokens) · compact above "
        "it with the reserve spent, because a checkpoint written with no headroom is "
        "thinner than the compaction it was meant to beat · UNKNOWN when occupancy could "
        "not be measured, which is deliberately NOT keep-going. DUE and READY are separate: "
        "the record must also read cold (every named slot filled, state leading with NEXT:, "
        "a checkpoint both taken and current) and --spawn REFUSES until it does. --spawn "
        "mints a session pre-attached to THE SAME task — no child, no new record — running "
        "the predecessor's own model selection ([1m] and all), launched with a continuation "
        "prompt generated from the RECORD and never the transcript. --force overrides the "
        "refusals; the gaps then travel INSIDE the prompt and the handoff is recorded as "
        "FORCED, because a degraded handoff is sometimes right and an invisible one never "
        "is. Every handoff is graded by the parent via `grade --handoff N`",
        "  grade  --task <child> --dim G1=A --dim G2=A- … [--threshold G] [--handoff N] "
        "[--note '…'] "
        ...
        "--handoff N grades session HANDOFF N (as `relay` numbered it) instead of the task's "
        "work — SAME rubric, SAME threshold, linked so a task that relayed more than once "
        "keeps its verdicts separate, and the handoff's mechanical evidence prints with the "
        "grade so a FORCED handoff cannot be scored without being seen",
