...
process whose cwd is inside the repo. This helper spawns such a process
(`claude -p`) so the work runs with the project's full machinery, keeps ONE
persistent worker per project (resuming it across turns), and relays the
worker's result back to the hub.

The decision of *when* to delegate lives in your CLAUDE.md (always in
context); this script is the *how*. Workers run with --permission-mode
acceptEdits and inherit each repo's allowlist (a tool the repo hasn't
allowlisted fails rather than prompting, since a headless child can't ask a
human).
