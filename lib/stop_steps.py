#!/usr/bin/env python3
"""stop_steps.py — every best-effort step of the Stop hook, in ONE interpreter.
...
one. The harness reads its stdout for the {"decision":"block"} contract, so it must
...
def main(argv=None):
    """Run every step in order, print only what the harness reads, always exit 0."""
    ...
    if "--session" in args:
        ...
        if i + 1 < len(args):
            session = args[i + 1]
    out = []
    for label, target, template in STEPS:
