...
| Decisions whose subject does not match their owner | **What turns the placement rule from etiquette into something reported.** Three tiers, and only the mechanical part is claimed: an **index** pointing at a ruling the source task does not confirm; an **owner** that no longer exists or is **closed**, so the ruling renders nowhere a live session looks; and a live decision **naming one other task three times and no other** while this task owns it. Three, because two occurrences are an ordinary cross-reference and that is what a decision log is *for*. The citational tier names a **proposal** in those words — it cannot know whether the ruling would outlive the child, and you can, in one question |
...
**The rule is one question: would this ruling still matter if this child were deleted?** Yes means the parent owns it; no means the child does. The `placement` check reports the part of that which is mechanical — a ruling naming one other task three times and no other, an owner that is gone or closed, an index the source denies — and says plainly that the rest is your judgement.
