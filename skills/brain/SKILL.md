...
1. **Search** with 2–3 term variants: `python3 -m brain.search search <terms>` (separate runs for synonyms; zero tokens). Add `--episodic` when the question is about past work/decisions ("when did we…", "why did we…"). Add `--snippets` for context. Also check the relevant hub in the vault's `projects/` (start from `INDEX.md` if unsure which).
2. **Read the top hits only** (notes are atomic — cheap). Trust order: `notes/` + `memory/` > `reports/` > `task-station/` > `raw/`. A `state` note beats any older-dated dashboard/wiki claim.
...
4. **Write back** (interactive sessions only): if the answer synthesized 3+ sources or is worth re-reading, file it to `reports/YYYY-MM-DD-<slug>.md` (schema per the vault CLAUDE.md), add an INDEX line, link it from the relevant hub, then `python3 -m brain.search log report <slug>` and commit the vault (`git -C "<vault>" add -A && git commit -m "brain: report <slug>"` — quote the path).
