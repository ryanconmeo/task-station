...
`delegate run` **streams** the worker's activity as it works — a compact feed
(`→ Edit: foo.py`, `· <assistant text>`, `✗` on tool errors) goes to **stderr**,
while stdout stays exactly the worker's final result (unchanged relay contract).

**For write or long-running delegations, launch with `run_in_background: true`** so
the run is inspectable via the down-arrow background-task inspector, which renders the
combined stdout+stderr feed live. Set **`PYTHONUNBUFFERED=1`** in the environment so
the feed isn't block-buffered when not attached to a tty. (A CLI can't register into
the internal `/workflows` renderer — this is the achievable equivalent of live
inspection.)

**Poll progress with `delegate status`** — it reports true liveness, not just a
timestamp:

```bash
python3 "$HOME/.claude/task-station-engine/delegate/delegate.py" status --seq <n>
# or --project <name> / --repo <path> / --label <l> / --all (bare `status` = --all)
```

Each entry shows a liveness glyph + state, the worktree git state (branch, last
commit, dirty/untracked counts), the last activity phase, and the resume command:

- `● running (quiet <N>s)` — pid alive (a quiet worker is still working, not dead).
- `○ not running — session resumable` — pid gone; **re-run the same command to resume**
  (never treat this as "dead/lost").
- `○ finished (ok|timeout|crash) <age> ago` — terminal state recorded.
- `? unknown` — a legacy pre-1.82 entry with no liveness info.

`delegate list` now prepends the same glyph per row.

**Auto-WIP-commit on abnormal exit.** If a worker exits abnormally — timeout,
non-zero exit, or the stream dying before a final result — `delegate`
auto-checkpoints the worktree with a `wip(delegate): auto-checkpoint on <reason>`
commit (`--no-verify`, **never pushed**) so in-progress edits are never orphaned, then
raises with the short sha (also in the `/todo` worker feed event). A **clean** finish is
left untouched — no auto-commit. Resume the same delegate command to continue; the WIP
commit is on the branch waiting for you.
...
- `delegate.py list` — show all known workers (with a liveness glyph) and their resume commands.
- `delegate.py status [--seq/--project/--repo/--label/--all]` — live per-worker liveness, git state, and phase (see *Live progress & observability*).
- `--fresh` — ignore the saved session and start a new one.
- `--label <slug>` — add a second concurrent worker for the same (task, repo).
- `--model <name>` — model for the worker (default `sonnet`; workers do author-only
  mechanical edits). Pass `opus` (or another stronger model) for genuinely hard work.
